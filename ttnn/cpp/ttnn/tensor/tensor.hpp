...
public:
    ...
    void print_info(const char* header) const {
        std::cout << header << std::boolalpha << " dtype " << magic_enum::enum_name(get_dtype()) << " layout "
                  ...
                  << " logical shape " << get_logical_shape().volume() << " padded rank " << get_padded_shape().rank()
                  << " padded shape " << get_padded_shape().volume() << " volume " << volume() << " logical volume "
                  << get_logical_volume() << " scalar? " << is_scalar() << " memory_layout "
                  << magic_enum::enum_name(memory_config().memory_layout) << " buffer_type "
